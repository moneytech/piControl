...
#endif
	    pr_info("Timeout because TX buffer is full %u %u  P1SR=0x%04x\n", pi, txmir, ksz8851_regrd(KSZ8851_P1SR));
	    // ksz8851_show();
	}
...
INT16U ksz8851ProcessInterrupt(void)
{
    ...
    {
#if 1
	pr_err("%d: ksz8851ProcessInterrupt: rx overrun 0x%x\n", spi_selected_chip(), isr);
	ksz8851ReInit();
    ...
    INT16U *pi16uLength_p)
{
	...
	pr_info_spi2("%d: ksz8851BeginPacketRetrieve abort1 0x%04d\n", spi_selected_chip(), tmp);
	return bFALSE;
    }
